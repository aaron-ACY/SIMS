...
import { instructorService, userService } from '../../../api/services';
...
const InstructorList = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    department: '', degree: 'Master', phone: ''
  });
  ...
  const handleAddInstructor = async (e) => {
    ...
    try {
      ...
      const res = await userService.createInstructor(formData);
      if (res.success || res.result) {
        ...
        alert(res.message || 'Failed to create instructor');
      }
    } catch (error) {
      console.error(error);
      alert(error.response?.data?.message || 'Error creating instructor');
    } finally {
  ...
  return (
    <div className="p-6 text-[var(--theme-text)] space-y-6">
      <PageHeader 
        ...
        description="Manage instructor records, specializations, and assignments."
        actions={
          <button 
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <table className="w-full text-left border-collapse bg-white">
              ...
              <tbody className="divide-y divide-[var(--theme-border)]">
                {instructors.map((instructor) => (
                  <tr key={instructor.instructorCode} className="hover:bg-[var(--theme-hover)]/30 transition-colors">
                    <td className="py-3 px-4 text-sm font-semibold text-[var(--theme-text)]">{instructor.instructorCode}</td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold text-[var(--theme-text)]">{instructor.fullName}</span>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4 text-sm text-[var(--theme-text)]">{instructor.degree}</td>
                    <td className="py-3 px-4 text-sm text-[var(--theme-text)]">{instructor.email}</td>
                    <td className="py-3 px-4 text-sm text-[var(--theme-text)]">{instructor.phone}</td>
                    <td className="py-3 px-4 text-sm text-[var(--theme-text)]">
                      <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${instructor.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            ...
            <EmptyState 
              ...
              description="There are currently no instructors in the system. Add a new instructor or import from CSV."
            />
          )}
        </div>
      </SectionCard>
      ...
      >
        <form onSubmit={handleAddInstructor} className="space-y-4">
          <p className="text-[var(--theme-textMuted)] text-sm mb-4">Create a new user account and instructor profile.</p>
